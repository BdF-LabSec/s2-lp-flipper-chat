...
typedef struct {
  ...
  uint8_t        cPreambleLength;    /*!< Set the preamble length in chips sequence (bit pairs) */
  uint8_t        cPostambleLength;   /*!< Set the postamble length in chips sequence (bit pairs) */
} PktWMbusInit;
