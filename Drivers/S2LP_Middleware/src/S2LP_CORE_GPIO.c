...
/**
 * @brief  Configures MCU GPIO and EXTI Line for GPIOs.
 *         This Ex version include the edge_direction as a parameter, to ovveride the configuration value.
 * @param  xGpio Specifies the GPIO to be configured.
 *         This parameter can be one of following parameters:
 *         @arg M2S_GPIO_0: GPIO_0
 *         @arg M2S_GPIO_1: GPIO_1
 ...
 * @retval None.
 */
void S2LP_Middleware_GpioInitEx(M2SGpioPin xGpio, M2SGpioMode xGpioMode, uint8_t edge_direction)
{
  GPIO_InitTypeDef GPIO_InitStructure;
...
void S2LPIRQEnableEx(uint8_t state, uint8_t edge_direction)
{
  S2LP_Middleware_GpioInitEx((M2SGpioPin)S2LP_GPIO_IRQ_PIN, M2S_MODE_EXTI_IN, edge_direction);

  S2LP_Middleware_GpioTriggerRising((M2SGpioPin)S2LP_GPIO_IRQ_PIN, edge_direction);
  S2LP_Middleware_GpioTriggerFalling((M2SGpioPin)S2LP_GPIO_IRQ_PIN, !edge_direction);
